...
void ClearScreenRightToLeft(char animation) {
    ...
    for(int i=SCREEN_WIDTH-1; i>=0;i--){
        for(int j=0; j<SCREEN_HEIGHT;j++){
            ...
            if(animation){
                if(i>0)
                    putch(j*64+i-1,0,BG_MAGENTA);
                for(volatile int wait=0;wait<WaitTime/5;wait++);
            }
        }
    }
    if(animation==0){
        for(volatile int wait=0;wait<WaitTime*50;wait++);
...
char houseTile_00[] = {
    ...
    while(1){
        ...
        if(renderedFrame==0){
            ...
            {
            case 0:
                printfSCR(SCREEN_WIDTH*(SCREEN_HEIGHT-1)+1,BG_WHITE,"%d",slide);
                printfSCR(SCREEN_WIDTH*5+10, BG_WHITE|0x5,  "  _____  _____  _____  _____   __     __");
                printfSCR(SCREEN_WIDTH*6+10, BG_WHITE|0x5,  " |  __ \\|_   _|/ ____|/ ____|  \\ \\   / /");    
                printfSCR(SCREEN_WIDTH*7+10, BG_WHITE|0x5,  " | |__) | | | | (___ | |   _____\\ \\_/ / ");
                printfSCR(SCREEN_WIDTH*8+10, BG_WHITE|0x5,  " |  _  /  | |  \\___ \\| |  |______\\   /  ");
                printfSCR(SCREEN_WIDTH*9+10, BG_WHITE|0x5,  " | | \\ \\ _| |_ ____) | |____      | |   ");
                printfSCR(SCREEN_WIDTH*10+10, BG_WHITE|0x5, " |_|  \\_\\_____|_____/ \\_____|     |_|   ");
                // printfSCR(SCREEN_WIDTH*11+15, BG_WHITE, "  
            break;
            case 1:
            
                printfSCR(SCREEN_WIDTH*(SCREEN_HEIGHT-1)+1,BG_WHITE,"%d",slide);
                printfSCR(SCREEN_WIDTH*7+24,BG_WHITE,  "What is RISC-Y?");
            break;
            case 2:
                printfSCR(SCREEN_WIDTH*(SCREEN_HEIGHT-1)+1,BG_WHITE,"%d",slide);
                printfSCR(SCREEN_WIDTH*7+20,BG_WHITE,  "A SoC written in verilog");
                printfSCR(SCREEN_WIDTH*8+23,BG_WHITE,     "System on a Chip");
            break;
            case 3:
                printfSCR(SCREEN_WIDTH*(SCREEN_HEIGHT-1)+1,BG_WHITE,"%d",slide);
                printfSCR(SCREEN_WIDTH*7+17,BG_WHITE,    "Continuation of the project from:"); 
                printfSCR(SCREEN_WIDTH*8+15,BG_WHITE,  "Computer Architecture and Organization"); 
                printfSCR(SCREEN_WIDTH*9+30,BG_WHITE,                 "ECE219"); 
            break;
            case 4:
                printfSCR(SCREEN_WIDTH*(SCREEN_HEIGHT-1)+1,BG_WHITE,"%d",slide);
                printfSCR(SCREEN_WIDTH*7+20,BG_WHITE,  "But instead of MIPS we have");
                printfSCR(SCREEN_WIDTH*8+23,BG_WHITE,     "RISC-V architecture");
            break;

            case 5:
                printfSCR(SCREEN_WIDTH*(SCREEN_HEIGHT-1)+1,BG_WHITE,"%d",slide);
                printfSCR(SCREEN_WIDTH*7+20,BG_WHITE,  "What can it do though?");
            break;
            ...
            case 6:
                ...
                printfSCR(0,BG_MAGENTA," It can display text, duh...\n");
                for(i=0;i<SCREEN_WIDTH;i++){
            ...
            case 7:
                ...
                printfSCR(SCREEN_WIDTH*(3+3),BG_WHITE,  "        It can do Simple Math!\n");
            ...
            case 8:
                ...
                printfSCR(SCREEN_WIDTH*(3+3)+8,BG_WHITE,  "It can have Basic Images!\n");
                printfSCR(SCREEN_WIDTH*(4+3)+12,BG_WHITE,  "-8x16 Sprites\n");
                printfSCR(SCREEN_WIDTH*(5+3)+12,BG_WHITE,  "-Whoping 4 colors!\n");
                printfSCR(SCREEN_WIDTH*(6+3)+12,BG_WHITE,  "-Something like this ->\n");
                    ...
                    putch(SCREEN_WIDTH*11+0  +35,128+0+3,BG_WHITE);
                    putch(SCREEN_WIDTH*11+1  +35,128+1+3,BG_WHITE);
                    putch(SCREEN_WIDTH*11+2  +35,128+2+3,BG_WHITE);
                    putch(SCREEN_WIDTH*11+3  +35,128+3+3,BG_WHITE);
                    putch(SCREEN_WIDTH*11+4  +35,128+4+3,BG_WHITE);
                    putch(SCREEN_WIDTH*11+5  +35,128+5+3,BG_WHITE);

                    putch(SCREEN_WIDTH*12+0  +35,128+ 0 +9,BG_WHITE);
                    // window1
                    putch(SCREEN_WIDTH*12+1  +35,128+ 1 +9,BG_WHITE);
                    // door 1
                    putch(SCREEN_WIDTH*12+2  +35,128+ 2 +9,BG_WHITE);
                    // door 2
                    putch(SCREEN_WIDTH*12+3  +35,128+ 3 +9,BG_WHITE);
                    // window2
                    putch(SCREEN_WIDTH*12+4  +35,128+ 4 +9,BG_WHITE);
                    putch(SCREEN_WIDTH*12+5  +35,128+ 5 +9,BG_WHITE);
                ...
                case 9:
                    ...
                    printfSCR(SCREEN_WIDTH*(3+3)+8,BG_WHITE,  "It can have animations!\n");
                    printfSCR(SCREEN_WIDTH*(4+3)+12,BG_WHITE,  "-GameBoy style animations. Based on sprites\n");
                    printfSCR(SCREEN_WIDTH*(5+3)+12,BG_WHITE,  "-Totally not copied this from pokemon\n");
                    printfSCR(SCREEN_WIDTH*(6+3)+12,BG_WHITE,  "-Look here ->\n");
                ...
                case 10:
                    ...
                    WriteSprite(3,    chair_00);
                    WriteSprite(4,    chair_01);
                    // WriteSprite(5,    indoorTree_00);
                    // WriteSprite(6,    indoorTree_01);
                    // WriteSprite(7,    indoorTree_10);
                    // WriteSprite(8,    indoorTree_10);
                    // WriteSprite(9,    indoorTree_11);
                    WriteSprite(10,    ragEnterance_00);
                    WriteSprite(11,    stairs_00);
                    WriteSprite(12,    stairs_01);
                    WriteSprite(13,   tv_00);
                    WriteSprite(14,   tv_01);
                    WriteSprite(15,   wall_00);
                    WriteSprite(16,   houseTile_00);

                    for(i=6;i<=12;i++){
                        for(int j=0;j<20;j++){
                            if(i==6){
                                putch(SCREEN_WIDTH*i+ j  +25,    128 + 15 ,0);
                            }
                            else{
                                putch(SCREEN_WIDTH*i+ j  +25,    128 + 16 ,0);
                            }
                        }
                    }
                    
                    // house rag
                    putch(SCREEN_WIDTH*12+2  +35,   128 + 10 ,0);
                    putch(SCREEN_WIDTH*12+3  +35,   128 + 10 ,0);
                    // stairs
                    putch(SCREEN_WIDTH*7+0  +43,   128 + 11 ,0);
                    putch(SCREEN_WIDTH*7+1  +43,   128 + 12 ,0);
                    // tv
                    putch(SCREEN_WIDTH*7+0  +36,    128 + 13 ,0);
                    putch(SCREEN_WIDTH*7+1  +36,    128 + 14 ,0);

                    // chair
                    putch(SCREEN_WIDTH*11+0  +35,    128 + 3 ,0);
                    putch(SCREEN_WIDTH*11+1  +35,    128 + 4 ,0);
                    // chair
                    putch(SCREEN_WIDTH*9+0  +35,    128 + 3 ,0);
                    putch(SCREEN_WIDTH*9+1  +35,    128 + 4 ,0);
                    // chair
                    putch(SCREEN_WIDTH*11+0  +39,    128 + 3 ,0);
                    putch(SCREEN_WIDTH*11+1  +39,    128 + 4 ,0);
                    // chair
                    putch(SCREEN_WIDTH*9+0  +39,    128 + 3 ,0);
                    putch(SCREEN_WIDTH*9+1  +39,    128 + 4 ,0);
                ...
                break;
                case 12:
                    printfSCR(SCREEN_WIDTH*6+5, BG_WHITE|0x05, "By:");
                    printfSCR(SCREEN_WIDTH*7+5, BG_WHITE|0x05, "P. Nanousis, I. Gaisidou, K. Varakliotis, D. Voitsidis");
                    printfSCR(SCREEN_WIDTH*9+5, BG_WHITE|0x05, "Thanks to:");
                    printfSCR(SCREEN_WIDTH*10+5, BG_WHITE|0x05, "A. Kastoras, A. Giannousas, I. Panagou and Dr. N. Mpellas");
                break;
            default:
                break;
            }
        ...
        if((currentFrame&1)==0){
            ...
            if(keys[0] == 'w'){
                ...
                ClearScreenRightToLeft(slide<8?1:0);
                slide++;
                renderedFrame=0;
            }
            else if(keys[0] == 'L'){
        ...
        if(((clock2-clock)>>1)>100000){
            ...
            if(slide==9){
                if(character00.x>=35*8&&character00.x<=40*8&&character00.y>=11*16&&character00.y<=12*16){
                    ClearScreenRightToLeft(0);
                    slide++;
                    renderedFrame=0;
                }
            }
