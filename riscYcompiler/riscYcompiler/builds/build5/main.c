...
int main() {
    ...
    ballObj.y = 150 << 8;     // Convert to fixed-point format
    ...
    for(volatile int i=0; i<WaitTime*200; i++);
    for(int i=0; i<SCREEN_WIDTH; i++){
        for(int j=0; j<SCREEN_HEIGHT; j++){
            putch(j*64+i,128+3,0);
        }
    ...
    while (1) {
        ...
        // active waiting
        for (volatile int i = 0; i < WaitTime * 4; i++);
       ...
       if ((ballObj.x >> 8) <= 0) {
            ...
            for(volatile int i=0; i<WaitTime*400; i++);
