...
int main() {
    ...
    while(1){
        ...
        clockcmp = *((volatile uint32_t *)MTIMECMP_ADDR);
        ...
        // printfSCR(SCREEN_WIDTH*13,15,"-%x_%x,-%x_%x",*((volatile uint32_t *)MTIME_ADDR+1),*((volatile uint32_t *)MTIME_ADDR),*((volatile uint32_t *)MTIMECMP_ADDR),);
